...
public class OcrServiceImpl implements OcrService {
    ...
    private List<ImportPreviewDTO> parseExcelForPreview(MultipartFile file) {
        try {
            // 使用 EasyExcel 读取。由于各种表格格式千奇百怪，我们这里必须包含表头一起读取，然后再从 list 中寻找真正的表头
            List<Map<Integer, String>> list = new ArrayList<>();
            EasyExcel.read(file.getInputStream()).sheet()
                     .headRowNumber(0) // 强制从第 0 行开始读（把表头也当成普通数据读进来）
            ...
            // 很多 Excel 首行可能是空行、大标题，真正的表头在后面
            // 我们在 processTabularData 中会自动寻找真正的表头行
    ...
    private List<ImportPreviewDTO> processTabularData(List<Map<Integer, String>> data) {
        ...
        for (int i = 0; i < Math.min(10, data.size()); i++) {
            ...
            // 如果某一行同时包含（名称）和（日期 或 展现量），大概率这就是表头
            if (hasName && (hasDate || hasImp)) {
                headers = row;
                headerRowIndex = i;
                log.info("在第 {} 行智能匹配到表头: {}", i, headers);
                break;
            }
        }

        if (headers == null) {
            log.error("无法在表格前10行中识别出有效表头！");
            return results; // 找不到表头，无法解析
        }
    ...
    private Long parseLongSafe(String value) {
        ...
        try {
            ...
            // 兼容 EasyExcel 可能会将纯数字识别为带 ".0" 的浮点数格式
            if (clean.contains(".")) {
    ...
    private List<ImportPreviewDTO> processOcrData(List<String> rawTexts) {
        ...
        // 2. 识别所有日期段
        List<DateSegment> segments = findDateSegments(fullText);
        log.info("识别到日期段数量: {}", segments.size());

        // 3. 提取所有可能的指标值（用于后续排除名称干扰）
        Set<String> allStatsValues = new java.util.HashSet<>();
        
        // 4. 处理每一行数据
        if (segments.isEmpty()) {
            // 回退到单行处理逻辑
            ImportPreviewDTO dto = processSingleSegment(fullText, null, allStatsValues);
            if (dto != null) results.add(dto);
        } else {
            for (DateSegment seg : segments) {
                ImportPreviewDTO dto = processSingleSegment(seg.content, seg.date, allStatsValues);
                if (dto != null) results.add(dto);
            }
        }

        // 5. 统一识别并填充名称
        String identifiedName = identifyName(fullText, allStatsValues);
        log.info("最终识别出的代码位名称: {}", identifiedName);

        if (identifiedName != null && !identifiedName.isEmpty()) {
            for (ImportPreviewDTO dto : results) {
                fillNameAndMapping(dto, identifiedName);
                calculateMetrics(dto);
            }
        } else {
            // 如果没识别出名称，至少也要计算一下指标
    ...
    private List<DateSegment> findDateSegments(String text) {
        ...
        java.util.regex.Pattern combinedPattern = java.util.regex.Pattern.compile(String.join("|", patterns));
        java.util.regex.Matcher m = combinedPattern.matcher(text);
        
        while (m.find()) {
            LocalDate date = parseDate(m.group());
            if (date != null) {
                DateSegment seg = new DateSegment();
                seg.date = date;
                seg.startPos = m.start();
                segments.add(seg);
            }
        }
        
        for (int i = 0; i < segments.size(); i++) {
            int end = (i + 1 < segments.size()) ? segments.get(i + 1).startPos : text.length();
            segments.get(i).content = text.substring(segments.get(i).startPos, end);
        }
        
        return segments;
    }

    private ImportPreviewDTO processSingleSegment(String text, LocalDate date, Set<String> allStatsValues) {
        ...
        while (numMatcher.find()) {
            ...
            try {
                if (s.contains(".")) {
                    ...
                    // 处理 OCR 误将千分位识别为小数点的情况 (如 84.063 -> 84063)
        ...
        if (candidates.isEmpty() && decimals.isEmpty()) return null;
        ...
        if (!candidates.isEmpty()) {
            Long imp = candidates.get(0);
            dto.setImpressions(imp);
            allStatsValues.add(imp.toString());
            allStatsValues.add(java.text.NumberFormat.getInstance().format(imp));
        }
        
        // 点击量：第二大整数，并尝试用 CTR 校验
        if (candidates.size() >= 2) {
            ...
            if (dto.getClicks() != null) {
                allStatsValues.add(dto.getClicks().toString());
            }
        }

        // 收入：最大的小数
        if (!decimals.isEmpty()) {
            BigDecimal rev = decimals.get(0);
            dto.setRevenue(rev);
            allStatsValues.add(rev.stripTrailingZeros().toPlainString());
            allStatsValues.add(rev.setScale(2, RoundingMode.HALF_UP).toString());
        }

        return dto;
    }

    private String identifyName(String fullText, Set<String> identifiedStatsValues) {
        ...
        for (String t : tokens) {
            ...
            if (cleanT.matches("^[0-9.%¥￥$]+$")) {
                ...
                if (val < 100 && !cleanT.equals("558")) continue; 
        ...
        for (String c : nameCandidates) {
            if (c.matches(".*[\\u4e00-\\u9fa5].*") && c.matches(".*[0-9].*")) return c;
        }
        for (String c : nameCandidates) {
            if (c.matches("^[\\u4e00-\\u9fa5]+$")) return c;
        }
        return nameCandidates.get(0);
    }

    private void fillNameAndMapping(ImportPreviewDTO dto, String identifiedName) {
